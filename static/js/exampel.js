...
var requestHandle = {
	...
	request:function(params,callback){
		...
		$.ajax({
		type : 'POST',
		url : 'http://biz.anyradio.cn/api/ServiceCenter.do',
		dataType : 'json',
		data : t._getParams(params),
	...
	_getParams:function(data){
        ...
        for(var i = 0,length = content.length;i<length;i++){
            var item = content[i];
            if(item.type==='text'){
                postInfo.push({type:item.type , text:encodeURIComponent(item.value)});
			}else{
            	let images = item.value;
            	for(var j = 0,count=images.length;j<count;j++){
            		let img = images[j];
                    postInfo.push({type:item.type , image:encodeURIComponent(img)});
				}
		...
		var specialdata = 'un='+sendPostParam.un+'&mid='+sendPostParam.mid+'&ptt='+sendPostParam.ppt+'&pct='+sendPostParam.pct+'&ian=true';
    ...
    _getimgParams:function(data){
        ...
        var sendPostParam = {un:attributes.un,pti:attributes.pti, phc:attributes.phc, phe:attributes.phe};
        var specialdata = 'un='+sendPostParam.un+'&pti='+sendPostParam.pti + '&phc='+ encodeURIComponent(sendPostParam.phc) + '&phe='+'jpg';
        return 'action='+this.imgaction+'&format=json&data='+this._xor(specialdata)+'&verf='+hex_md5(encodeURIComponent(specialdata))+'&pv=3';
    },
	_xor:function(data){
		data = encodeURIComponent(data);
		var orData = '';
		for(var i=0;i<data.length;i++){
			orData += String.fromCharCode((data.substr(i,1).charCodeAt() ^ 0x6e));
		}
		data = this._base64encode(orData);
		data = encodeURIComponent(data);
		return data;
	},
	_base64encode(str){
